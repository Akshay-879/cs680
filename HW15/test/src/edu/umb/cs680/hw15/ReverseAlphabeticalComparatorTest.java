...
class  ReverseAlphabeticalComparatorTest {
    ...
    @Test
    void verifyreversecomparatorforhome() {
        ...
        for(ApfsElement Apfselement : root.getSubDirectories().get(1).getFiles((ApfsElement object1, ApfsElement object2) -> object2.getName().compareTo(object1.getName()))){
    ...
    void verifyreversecomparatorforroot() {
        APFS ApfsFileSystem = APFS.getAPFSFileSystem();
        ApfsDirectory root = (ApfsDirectory)ApfsFileSystem.getRootDir();
        String[] expected = {"home","code", "applications"};
        assertArrayEquals(expected,dirToStringArray(root.getChildren((ApfsElement object1, ApfsElement object2) -> object2.getName().compareTo(object1.getName()))));
    }
